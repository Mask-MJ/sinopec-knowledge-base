 ...
 *   2026-09-06 由 923 字扩到 1598 字（v7），对照数据见
...
2. **数字必须精确严格，每个数值都要带单位**：所有数字（井号、坐标、限差、面积、覆盖次数、炮数、控制点编号等）必须照实给出，包含正负号。**单位一律不得省略**——哪怕同一段里前文已经出现过该单位、哪怕单位在专业上不言自明、哪怕是分条列举，每个数值仍要各自带上单位。例如闭合差写"0.059m"而不是"0.059"，井深写"8-12m"而不是"8-12"，覆盖次数写"900次"而不是"900"。**漏写单位等同于答错。**
